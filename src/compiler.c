...
#   if STATIC_TYPING()
    ...
    if (method.isSingle) {
      ASSERT(method.as.one != NULL, "Single method cannot be NULL");
      newSignature = *method.as.one;
      printf("%p\n", method.as.one->returnType);
    } else {
      if (!getSignature(
        method.as.list,
        signature.arity,
...
static void lambda(bool canAssign) {
  ...
  if (match(TOKEN_LEFT_PAREN)) {
    expect(TOKEN_RIGHT_PAREN, "Expecting ')' for empty parameter list");
  } else {
    do {
